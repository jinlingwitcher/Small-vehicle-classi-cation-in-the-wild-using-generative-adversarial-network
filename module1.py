...
trans_lr ,trans_hr = tf(high_resolution, high_resolution)





weights_ = torch.load("./models/generator_1499.pth")
weights = OrderedDict()
...
img = Image.open("./imgs/00001.jpg") #?*?*3
#imgg= np.array(img)
img = img.convert('RGB')

img_lr = trans_lr(img).unsqueeze(0)  #1 3 32 32

img_hr = trans_hr(img).unsqueeze(0)  #1 3 128 128
print(img_lr)
...
img_hr = torch.clamp(img_hr*0.5 + 0.5, 0, 1)
...
#plt.savefig('l_256.png')
plt.show()
matplotlib.image.imsave("02.jpg",hrp)
